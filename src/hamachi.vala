...
public class Hamachi : Object
{
    ...
    {
        string output = Command.return_output (Command.sudo + " " + Command.sudo_start + Utils.format (service, "restart", null, null));
    ...
    public static void restore_config (string filename)
    {
        ...
        if (output.contains (Hamachi.data_path))
        {
            ...
            output = Command.return_output (Command.sudo + " " + Command.sudo_start + "bash -c \"" + Utils.format (service, "start", null, null) + "; " + Utils.format (service, "stop", null, null) + "; killall -9 hamachid &> /dev/null; rm " + Hamachi.data_path + "/*; tar -xavf '" + file_path + "' -C /; " + Utils.format (service, "start", null, null) + "; sleep 1\"");
